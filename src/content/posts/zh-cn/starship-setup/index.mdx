...
tags:
    ...
    - Nushell
description: >-
    Starship 是一款开源的 shell 主题管理器，其支持 Zsh、Nushell、PowerShell 等。
...
<Callout title="笔者的配置" type="info" collapsible={true} open={false} client:idle>
...
`format` 必须在所有组件之前定义，否则其会被当作组件的 `format` 字段 [^toml-table]。
...
<Callout type="info" title="组件之间的空格">

默认情况下，组件之间的空格分散在各个组件的 `format` 字段中，但笔者推荐在全局 `format` 字段中添加空格，以便集中管理。
